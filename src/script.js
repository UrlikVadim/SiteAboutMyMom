...
function addComents(data) {
    ...
    if (accesskey == null && clickMod == false) {
        ...
        for (i = 0; i < dataarray.length - 3; i += 4) {
            ...
            var strAdd = '<div class = "Comment"><div class = "nameComment">' + dataarray[i] + '</div><div class = "textComment" id = "kek-' + i + '">' + dataarray[i + 1] + '</div><div id = "commentId-' + dataarray[i + 2] + '"><div class = "deleteComment">Удалить</div><div class = "visibleComment">' + visible + '</div></div></div>';
            $("#Comments").append(strAdd);
            var lol = "#kek-" + i;
            $(lol).val(dataarray[i + 1]);
        }
